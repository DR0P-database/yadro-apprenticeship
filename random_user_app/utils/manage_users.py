...
from random_user_app.orm.models import (
    DOB,
    Coordinates,
    IDInfo,
    Location,
...
async def get_new_users(session: AsyncSession, count: int = 1) -> list[UserInDB]:
    ...
    if count > settings.LIMIT_USERS_PER_PAGE:
        total_pages = (
            count + settings.LIMIT_USERS_PER_PAGE - 1
        ) // settings.LIMIT_USERS_PER_PAGE
    ...
    while response["info"]["page"] < total_pages:
        ...
        response = await fetch_users(
            count, page=response["info"]["page"] + 1, seed=seed
        )
        ...
        logger.info(
            f"Saving {len(api_users)} users to DB from page {response['info']['page']}"
        )
        for user in api_users:
            user, status = await save_user_to_db(UserInDB.model_validate(user), session)
            saved_users += status
    ...
    logger.info(
        f"Finished: {saved_users} users saved, {fetched_count_users - saved_users} users skipped"
    )
...
async def save_user_to_db(user: UserInDB, db: AsyncSession):
    ...
    location = Location(
        **user.location.model_dump(
            exclude={"street", "coordinates", "timezone", "postcode"}
        ),
    ...
    db_user = Users(
        **user.model_dump(
            exclude={"location", "name", "login", "dob", "registered", "id", "picture"}
        ),
        name_id=name.id,
...
async def get_users_from_db(session: AsyncSession, limit: int = 1, offset: int = 0):
    ...
    if not users:
        return GetAllUsers.model_validate(
            {"users": [], "limit": limit, "offset": offset, "total": 0}
        )
