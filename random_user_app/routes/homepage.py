...
from random_user_app.utils.manage_users import fetch_users, get_users_from_db, save_user_to_db
...
async def get_random_user(request: Request, session: AsyncSession = Depends(get_session)):
    ...
    # user['id_info'] = user.pop('id')
    
    user, status = await save_user_to_db(UserInDB.model_validate(user), db=session)


    return templates.TemplateResponse(name='userpage/userpage.html', context={'request': request, 'user': user})
